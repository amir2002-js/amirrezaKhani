...
export default function Range({ maxNum, setMinShow, numOfShow }) {
	...
	for (let i = -1; i < 2; i++) {
		chosen + i > 2 &&
			...
			chosen != 1 &&
			// (chosen == 2 && maxNum < 2)
			valOfBtn.push({ val: chosen + i, id: chosen + i });

		chosen + i > 2 &&
			chosen + i < maxNum &&
			chosen != 1 &&
			chosen != 2 &&
			console.log({ val: chosen + i, id: chosen + i });
	}

	chosen + 1 < maxNum - 1 && valOfBtn.push({ val: "...", id: -2 });
	...
	return (
		<div className=" mx-auto flex justify-center items-center gap-5 max-sm:gap-2">
			<IncDecBtn action="-" setChosen={changeChosen} chosen={chosen}>
				<BsChevronLeft />
			</IncDecBtn>
			...
			{valOfBtn.map((i) => (
				<button
					...
					id={i.id}
					className={`p-2 px-3 ${
						i.val == chosen ? "bg-black text-white" : "bg-gray-200"
